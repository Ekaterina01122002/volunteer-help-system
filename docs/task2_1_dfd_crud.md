...
```mermaid
flowchart LR
    C["Координатор"] -->|заявка, волонтер| P31(("3.1 Проверить доступность"))
    P31 -->|валидные данные| P32(("3.2 Записать назначение"))
    P32 -->|обновленная заявка| P33(("3.3 Изменить статус"))
    P33 -->|история статуса| P34(("3.4 Сохранить данные"))
    D1[("D1 Заявки")]
    D2[("D2 Волонтеры")]
    P31 -->|читать волонтера| D2
    P32 -->|записать volunteerId| D1
    P34 -->|заявка в работе| D1
```
